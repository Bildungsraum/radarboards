...
xRendererTechUDimEinzel.labels.template.setAll({
  ...
  fill: am5.Color.brighten(ColorBlack, 0.4),
  // fontFamily: "Verdana",
  fontFamily: "Verdana",
  fontWeight: fontWeightLabelAussen,
...
xRendererTechUDimEinzel.grid.template.setAll({
  ...
  //stroke: ColorWhite,
  strokeWidth: 0.10 ///Bestimmt dicke und farbe des grids
});
...
function erstelleSeriesEinzelTechUDimEinzel(index) { /// index= Index für Projekt geht von 1..34
  ...
  seriesEinzelProjekt[index].strokes.template.setAll({ strokeWidth: 2.0, fillOpacity: 0, fill: color }); //war fillOpacity: 0.2 weg wegen grau zonen
...
seriesUmschaltenFachlich.on("visible", function (visible, target) {
  if (visible) {
    ...
    window.location.href = "einzel-udim.html";
  }
});
